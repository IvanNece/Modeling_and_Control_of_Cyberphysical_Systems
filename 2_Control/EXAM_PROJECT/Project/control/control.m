function [K, c, F, L1, A0_obv, B0_obv, C0_obv, D0_obv, Aa_obv, Ba_obv, Ca_obv, Da_obv] = control(A, B, C, K0, L, G, p)
    ...
    % Ora include osservatori separati per LEADER e FOLLOWER
    ...
    P2 = care((A - B * K0)', C' * inv(R_obs) * C, Q_obs); % Soluzione dell'equazione di Riccati per F
    F = P2 * C' * inv(R_obs); % Guadagno dell'osservatore
    ...
    B0_obv = [L1 B]; % Matrice di ingresso per l'osservatore del leader
    C0_obv = eye(2); % Uscita per il leader osservato
    D0_obv = zeros(2); % Componente di uscita per il leader
    ...
    Da_obv = zeros(2); % Componente di uscita per gli agenti
end